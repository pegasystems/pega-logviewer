...
public class EVENTReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(EVENTReportModel.class);

    private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

    public EVENTReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Event Application - Event Code\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (dataText != null) {
            String[] eventDataArray = dataText.split("\\|", 0);

            if (eventDataArray.length > 2) {

                StringBuilder keySB = new StringBuilder();

                keySB.append(eventDataArray[0]);
                keySB.append("-");
                keySB.append(eventDataArray[1]);
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("EVENTReportModel - Could'nt match - [" + message + "]");
        }
