...
public class PEGA0072ReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0072ReportModel.class);
    ...
    public PEGA0072ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {
        ...
        regex = "Run \\[(.*)\\] with data flow \\[(.*)\\] has failed";
        oldPattern = Pattern.compile(regex);
    }
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Dataflow Name - Failed\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (matches) {
            ...
            Matcher oldPatternMatcher = oldPattern.matcher(dataText);
            matches = oldPatternMatcher.find();

            if (matches) {

                alertMessageReportEntryKey = oldPatternMatcher.group(2).trim();

                if ((alertMessageReportEntryKey == null) || ("".equals(alertMessageReportEntryKey))) {
                    alertMessageReportEntryKey = oldPatternMatcher.group(1).trim();
                }
            }
        }
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("PEGA0072ReportModel - Could'nt match - [" + message + "]");
        }
