...
public class PEGA0134ReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0134ReportModel.class);

    private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

    private Pattern pattern;

    public PEGA0134ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {

        super(alertMessage, thresholdKPI, alertLogEntryModel, locale);

        String regex = "Number of items in kafka topic for Queue Processor named (.*?) has exceeded its threshold.*";
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Queue Processor\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (matches) {
            alertMessageReportEntryKey = patternMatcher.group(1);
        }
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("PEGA0134ReportModel - Could'nt match - [" + message + "]");
        }
