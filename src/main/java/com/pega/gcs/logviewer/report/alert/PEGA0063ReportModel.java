...
public class PEGA0063ReportModel extends AlertMessageReportModel {

	private static final long serialVersionUID = -8889727175209305065L;

	private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0063ReportModel.class);

	private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

	private Pattern pattern;

	public PEGA0063ReportModel(long thresholdKPI, String kpiUnit, AlertLogEntryModel alertLogEntryModel) {

		super("PEGA0063", thresholdKPI, kpiUnit, alertLogEntryModel);

		String regex = "For strategy shape \\[(.*?)\\] in data flow \\[(.*?)\\]";
	...
	protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
		...
		if (alertMessageReportColumnList == null) {
			...
			displayName = "Strategy [Data Flow]";
			prefColWidth = 500;
	...
	public String getAlertMessageReportEntryKey(AlertLogEntry alertLogEntry, ArrayList<String> logEntryValueList) {
		...
		if (matches) {
			...
			String dataflow = patternMatcher.group(2).trim();
		...
		if (alertMessageReportEntryKey == null) {
			LOG.info("PEGA0063ReportModel - Could'nt match - [" + message + "]");
		}
	...
	public static void main(String[] args) {
		...
		String message1 = "Strategy record processing time exceeds threshold of 5000 ms: 5001 ms. For strategy shape [metrics.getStageName()] in data flow [alertParameters.getDataflowName()]";
