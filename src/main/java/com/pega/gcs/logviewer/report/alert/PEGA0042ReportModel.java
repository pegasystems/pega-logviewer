...
public class PEGA0042ReportModel extends AlertMessageReportModel {

	private static final long serialVersionUID = -8889727175209305065L;

	private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0042ReportModel.class);

	private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

	private Pattern pattern;

	public PEGA0042ReportModel(long thresholdKPI, String kpiUnit, AlertLogEntryModel alertLogEntryModel) {

		super("PEGA0042", thresholdKPI, kpiUnit, alertLogEntryModel);
	...
	public String getAlertMessageReportEntryKey(AlertLogEntry alertLogEntry, ArrayList<String> logEntryValueList) {
		...
		if (alertMessageReportEntryKey == null) {
			LOG.info("PEGA0042ReportModel - Could'nt match - [" + message + "]");
		}
	...
	public static void main(String[] args) {
		...
		String message1 = "Packaging of database query took more than threshold of 500 ms: 1,323 ms SQL: SELECT PYUSERNAME AS \"pyUserName\" , PYUSERIDENTIFIER AS \"pyUserIdentifier\" , PYACCESSGROUP AS \"pyAccessGroup\" , ACCESSROLE AS \"AccessRole\" , PXCREATEDATETIME AS \"pxCreateDateTime\" , DISABLED AS \"disabled\" , PYREPORTTO AS \"pyReportTo\" , HUBSKILL AS \"HUBSkill\" , LEGALENTITYSKILL AS \"LegalEntitySkill\" , CLIENTTYPESKILL AS \"ClientTypeSKill\" , ACCESSROLE AS \"AccessRole\" , TEAMIDENTIFIER AS \"TeamIdentifier\" , DATEOFDEACTIVATION AS \"DateOfDeactivation\" , JURISDICTIONSKILL AS \"Jurisdictionskill\" , PYACCESSGROUPADDITIONAL AS \"PYACCESSGROUPADDITIONAL\" FROM HSBC_NGT_NGTUSER_VW WHERE ( PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? OR PYACCESSGROUP = ? ) AND ( pxObjClass = ? )";
