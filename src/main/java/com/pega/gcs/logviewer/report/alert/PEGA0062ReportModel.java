...
public class PEGA0062ReportModel extends AlertMessageReportModel {
    ...
    private Pattern slowestStagePattern;

    public PEGA0062ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {
        ...
        String slowestStageRegex = "Slowest stage \\[(.*?)\\]";
        slowestStagePattern = Pattern.compile(slowestStageRegex);
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Data Flow [Slowest Stage]\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (matches) {
            ...
            Matcher slowestStageMatcher = slowestStagePattern.matcher(dataText);
            matches = slowestStageMatcher.find();

            if (matches) {

                String slowestStage = slowestStageMatcher.group(1).trim();

                StringBuilder sb = new StringBuilder();

                sb.append(alertMessageReportEntryKey);
                sb.append(" [");
                sb.append(slowestStage);
                sb.append("]");
