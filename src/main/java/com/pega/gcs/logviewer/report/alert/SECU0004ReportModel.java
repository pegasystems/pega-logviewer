...
public class SECU0004ReportModel extends AlertMessageReportModel {

	private static final long serialVersionUID = -8889727175209305065L;

	private static final Log4j2Helper LOG = new Log4j2Helper(SECU0004ReportModel.class);

	private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

	private Pattern pattern;

	public SECU0004ReportModel(long thresholdKPI, String kpiUnit, AlertLogEntryModel alertLogEntryModel) {

		super("SECU0004", thresholdKPI, kpiUnit, alertLogEntryModel);

		String regex = " \\:(.*?)was not allowed";
	...
	public String getAlertMessageReportEntryKey(AlertLogEntry alertLogEntry, ArrayList<String> logEntryValueList) {
		...
		if (alertMessageReportEntryKey == null) {
			LOG.info("SECU0004ReportModel - Could'nt match - [" + message + "]");
		}
	...
	public static void main(String[] args) {
		...
		String message1 = "Attempt to run a stream from URL failed in web node environment : Stream 'pyStream' was not allowed to run in WEB node because direct stream execution is not supported in this environment";
