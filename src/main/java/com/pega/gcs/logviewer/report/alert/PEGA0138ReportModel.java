...
public class PEGA0138ReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0138ReportModel.class);

    private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

    private Pattern pattern;

    public PEGA0138ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {

        super(alertMessage, thresholdKPI, alertLogEntryModel, locale);

        String regex = "Could not apply cloud optimization to simulation \\[(.*?)\\] using strategy rule \\[(.*?)\\].*";
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Strategy\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (matches) {
            alertMessageReportEntryKey = patternMatcher.group(2);
        }
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("PEGA0138ReportModel - Could'nt match - [" + message + "]");
        }
