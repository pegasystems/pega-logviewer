...
public class PEGA0073ReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0073ReportModel.class);
    ...
    public PEGA0073ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
        ...
        String regex = "Run \\[(.*)\\] with data flow \\[(.*)\\] encountered an error: \\[(.*)\\]";
        pattern = Pattern.compile(regex);

        regex = "^\\[(.*?)\\] [Preparing retry|Maximum retries|Dataflow is stopping].*";
        retryPattern = Pattern.compile(regex);
    ...
    protected List<AlertBoxAndWhiskerReportColumn> getAlertMessageReportColumnList() {
        ...
        if (alertMessageReportColumnList == null) {
            ...
            displayName = "Alert Subject (\"Dataflow Run Id\")";
            prefColWidth = 500;
    ...
    public String getAlertMessageReportEntryKey(String dataText) {
        ...
        if (matches) {
            ...
            Matcher retryPatternMatcher = retryPattern.matcher(dataText);
            matches = retryPatternMatcher.find();

            if (matches) {
                alertMessageReportEntryKey = retryPatternMatcher.group(1).trim();
            } else {

                Matcher errorPatternMatcher = errorPattern.matcher(dataText);
                matches = errorPatternMatcher.find();
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("PEGA0073ReportModel - Could'nt match - [" + message + "]");
        }
