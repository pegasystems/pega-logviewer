...
public class PEGA0142ReportModel extends AlertMessageReportModel {

    private static final long serialVersionUID = -8889727175209305065L;

    private static final Log4j2Helper LOG = new Log4j2Helper(PEGA0142ReportModel.class);

    private List<AlertBoxAndWhiskerReportColumn> alertMessageReportColumnList;

    private Pattern pattern;

    public PEGA0142ReportModel(AlertMessage alertMessage, long thresholdKPI, AlertLogEntryModel alertLogEntryModel,
            Locale locale) {

        super(alertMessage, thresholdKPI, alertLogEntryModel, locale);

        String regex = "Stuck emails reached limit while processing for listener (.*?) with.*";
    ...
    public String getAlertMessageReportEntryKey(ArrayList<String> logEntryValueList) {
        ...
        if (alertMessageReportEntryKey == null) {
            LOG.info("PEGA0142ReportModel - Could'nt match - [" + message + "]");
        }
