...
public class AlertBoxAndWhiskerItem extends BoxAndWhiskerItem implements Comparable<AlertBoxAndWhiskerItem> {
    ...
    @Override
    public int compareTo(AlertBoxAndWhiskerItem other) {

        // Double thisTotal = Double.valueOf(getMaxOutlier().toString());
        // Double otherTotal = Double.valueOf(other.getMaxOutlier().toString());
        //
        // int result = thisTotal.compareTo(otherTotal);

        Double thisMax = Math.max(getMaxOutlier().doubleValue(), getMaxRegularValue().doubleValue());
        Double otherMax = Math.max(other.getMaxOutlier().doubleValue(), other.getMaxRegularValue().doubleValue());
        int result = thisMax.compareTo(otherMax);

        if (result == 0) {
            Integer thisCount = Integer.valueOf(getCount());
            Integer otherCount = Integer.valueOf(other.getCount());
